...
{
 const {ev,fire}=game();ev("S.s('ccIntro10',1);state='title'");fire('keydown',{code:'Enter'});if(ev('level')!==0||ev('state')!=='play')throw Error('revised tutorial not forced');
}
{
 const {ev,fire,click}=game();ev('startLevel(0)');if(ev('LM.length')!==1||ev("LM[0].k")!=='bakery')throw Error('tutorial landmark');fire('keydown',{code:'KeyD'});ev('updateWorld(.1)');if(ev('intro.step')!==0)throw Error('move skipped release lesson');fire('keyup',{code:'KeyD'});ev('update(.02)');if(ev('intro.step')!==1||ev('unis[0].order')<=0)throw Error('route release lesson');click();ev('update(.02)');if(!ev('intro.r')||ev('intro.step')!==2||ev('unis[caps[1]].tap')!==1)throw Error('whip first crack');click();ev('update(.02)');if(ev('intro.dash')||ev('intro.step')!==2||ev('unis[caps[1]].tap')!==2)throw Error('chain graduated before 3x');click();ev('update(.02)');if(!ev('intro.dash')||ev('intro.step')!==3)throw Error('3x chain lesson');ev('intro.power=1;updateIntro(.1);intro.distract=1;updateIntro(.1);intro.rescue=1;updateIntro(.1);LM[0].os.forEach(o=>o.hp=0);update(.016)');if(ev('state')!=='between'||ev("S.g('ccIntro16')")!==1)throw Error('tutorial objective/onboarding gate');
}
{
...
console.log('headless v0.16 guided onboarding smoke: PASS');
