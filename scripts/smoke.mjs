...
{
 const {ev,fire,press}=game();ev('startLevel(0)');let first=ev('caps[0]');fire('keydown',{code:'ShiftLeft'});if(ev('caps[0]')!==first)throw Error('tutorial must teach in order');fire('keydown',{code:'KeyD'});ev('updateWorld(.1)');ev('updateIntro()');if(ev('intro.step')!==1)throw Error('WASD lesson');fire('keyup',{code:'KeyD'});if(ev('caps[0]')!==first||ev('unis[0].order')>0)throw Error('WASD release must not switch');for(let i=0;i<5;i++)press();if(ev('unis[0].power')!==5||ev('intro.step')!==2)throw Error('Bolt must reach five-whip charge');fire('keydown',{code:'Space'});if(ev('unis[0].power')!==0||ev('intro.step')!==3||ev('unis[0].dash')<=0)throw Error('Bolt dash lesson');let d1=ev('unis[0].dash');fire('keydown',{code:'ShiftLeft'});if(ev('caps[0]')!==1||ev('intro.step')!==4||ev('unis[0].order')<=0)throw Error('Shift must hand off to Mallow and leave Bolt running');for(let i=0;i<5;i++)press();if(ev('unis[1].power')!==5||ev('intro.step')!==5)throw Error('Mallow must reach five-whip charge');ev('update(.5)');fire('keydown',{code:'Space'});if(ev('state')!=='play'||ev('intro.step')!==6||ev('unis[1].dash')<=0)throw Error('second dash must be visible before briefing');ev('update(.8)');if(ev('state')!=='between'||!ev("S.g('ccIntro16')")||!ev("S.g('ccIntro10')")||d1<=0)throw Error('second dash beat must open town briefing');
}
{
...
console.log('headless v0.23 one-captain five-whip training + highways: PASS');
