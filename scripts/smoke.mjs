...
{
 const {ev,fire}=game();ev('startLevel(1)');if(ev('unis.filter(u=>u.live).length')!==6)throw Error('real runs must start with full herd');let seen=new Set([ev('caps[0]')]);for(let i=0;i<5;i++){fire('keydown',{code:'ShiftLeft'});seen.add(ev('caps[0]'))}if(seen.size!==6)throw Error('Shift must freely visit every live unicorn');fire('keydown',{code:'ShiftLeft'});if(ev('caps[0]')!==0)throw Error('full-herd Shift cycle must wrap');
}
{
