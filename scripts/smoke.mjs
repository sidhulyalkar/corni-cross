...
{
 const {ev,fire}=game();ev("S.s('ccIntro10',1);state='title'");fire('keydown',{code:'Enter'});if(ev('level')!==0||ev('state')!=='play')throw Error('fresh solo tutorial not forced');if(ev('caps[0]')!==0||ev('caps[1]')!==-1||ev('unis.filter(u=>u.live).length')!==2)throw Error('tutorial must expose two unicorns but one captain');
}
{
 const {ev,fire,press}=game();ev('startLevel(0)');let first=ev('caps[0]'),bak=ev('LM[0].os[0].hp');ev('hitObj(unis[0],LM[0].os[0],999)');if(ev('LM[0].os[0].hp')!==bak)throw Error('bakery must wait for final tutorial beat');fire('keydown',{code:'ShiftLeft'});if(ev('caps[0]')!==first)throw Error('tutorial must teach in order');fire('keydown',{code:'KeyD'});ev('updateWorld(.1)');if(ev('intro.step')!==1)throw Error('WASD lesson');fire('keyup',{code:'KeyD'});for(let i=0;i<5;i++)press();if(ev('unis[0].power')!==5||ev('intro.step')!==2)throw Error('Bolt must reach five-whip charge');fire('keydown',{code:'Space'});if(ev('unis[0].power')!==0||ev('intro.step')!==3||ev('unis[0].dash')<=0)throw Error('Bolt dash lesson');fire('keydown',{code:'ShiftLeft'});if(ev('caps[0]')!==1||ev('intro.step')!==4||ev('unis[0].order')<=0)throw Error('Shift must hand off to Mallow and leave Bolt running');fire('keydown',{code:'KeyD'});ev('updateWorld(.12)');fire('keyup',{code:'KeyD'});if(ev('intro.step')!==5||ev('suggested')!==0||ev('unis[0].distract')<.5)throw Error('mini-town must demonstrate distracted Bolt recommendation');fire('keydown',{code:'ShiftLeft'});if(ev('caps[0]')!==0||ev('intro.step')!==6)throw Error('Shift must answer distraction recommendation');fire('keydown',{code:'KeyD'});ev('updateWorld(.1)');fire('keyup',{code:'KeyD'});if(ev('intro.step')!==7||!ev('intro.rescue'))throw Error('taking control must teach distraction rescue');for(let i=0;i<5;i++)press();if(ev('unis[0].power')!==5||ev('intro.step')!==8)throw Error('final max dash charge');fire('keydown',{code:'Space'});if(ev('unis[0].dash')<=0||ev('intro.step')!==8)throw Error('final bakery dash must launch');ev('hitObj(unis[0],LM[0].os[0],999);update(.016)');if(ev('state')!=='between'||!ev("S.g('ccIntro16')")||!ev("S.g('ccIntro10')"))throw Error('bakery smash must finish training and unlock town');
}
{
...
console.log('headless v0.23 guided mini-town tutorial + controls: PASS');
