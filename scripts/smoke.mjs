...
{
 const {ev}=game();ev("S.s('ccC2',1);S.s('ccHeat',0);startLevel(1)");if(!ev('plus')||ev('heat')!==0||ev('timeLeft')!==114||ev('cleaners.length')!==1||!ev("lmText.includes('HEAT 1')"))throw Error('Stampede+ Heat 1 unlock');if(Math.abs(ev('chaos()')-.28)>1e-6)throw Error('Heat 1 chaos target');ev('unis[0].x=550;unis[0].y=1130;updateZone(.01)');if(!(ev('dmask')&64)||ev('unis[0].boost')<2)throw Error('prism gate reward');ev('victoryT=.01;landWin=1;update(.02)');if(ev("S.g('ccHeat')")!==1)throw Error('winning must advance Heat');
}
{
...
let ws=fs.readFileSync('src/whip.js','utf8'),es=fs.readFileSync('src/expansion.js','utf8');if(!ws.includes('X.arc(-7,7,14+i*7,.2,5.6)')||!ws.includes('quadraticCurveTo')||ws.includes('*700-56')||!ws.includes('whip.l*520'))throw Error('snap whip regression');if(!es.includes('u.h+120')||!es.includes("lmText+=' • +6s'"))throw Error('mane/time-bank regression');
if(!fs.readFileSync('src/style.css','utf8').includes('cursor:none'))throw Error('native cursor not hidden');
console.log('headless v0.26 snap whip + manes + strategic time bank: PASS');
