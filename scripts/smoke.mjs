...
function game(){
 ...
 const box={console,Math,Date,Uint8Array,AudioContext,localStorage:{},document:{getElementById:()=>canvas,createElement:()=>({width:0,height:0,getContext:()=>ctx})},addEventListener:(n,f,c)=>on(n,f,c),requestAnimationFrame:()=>{}};vm.createContext(box);vm.runInContext(['core.js','herd.js','render.js','ui.js','top10.js','polish.js','hud12.js','whip.js'].map(f=>fs.readFileSync('src/'+f,'utf8')).join('\n'),box);
...
{
 const {ev,fire,click}=game();ev('startLevel(1)');if(ev('LM.length')!==5)throw Error('landmark count');let hp=ev("LM[4].os[0].hp");ev("hitObj(unis[0],LM[4].os[0],999)");if(ev("LM[4].os[0].hp")!==hp)throw Error('town hall shield');ev("LM.slice(0,-1).forEach(l=>l.os.forEach(o=>o.hp=0));hitObj(unis[0],LM[4].os[0],10)");if(ev("LM[4].os[0].hp")>=hp)throw Error('town hall unlock');ev('runTime=19;update(.016)');if(ev('wave')!==2||ev('cleaners.length')!==2)throw Error('act2 cleanup');fire('keydown',{code:'KeyD'});ev('updateWorld(.1)');fire('keyup',{code:'KeyD'});if(ev('caps[0]')!==1||ev('unis[0].order')<3)throw Error('left smart next');click();click();if(ev('unis[caps[1]].tap')!==2||ev('unis[caps[1]].boost')<1)throw Error('2x prism');click();if(ev('caps[1]')!==4||ev('unis[3].order')<3)throw Error('3x smart next');ev('update(.016)');if(ev('wave')!==3||ev('cleaners.length')!==4)throw Error('act3');ev('caps[0]=0;unis[1].order=3;unis[2].distract=1;cycle(0)');if(ev('caps[0]')!==2||ev('cueWhy[0]')!==1)throw Error('director reason');ev('hud()');
}
{
...
console.log('headless v0.12 reserve smoke: PASS');
