...
{
 const {ev,fire,click}=game();ev('startLevel(0)');if(ev('LM.length')!==1||ev("LM[0].k")!=='bakery')throw Error('tutorial landmark');fire('keydown',{code:'KeyD'});ev('updateWorld(.1)');fire('keyup',{code:'KeyD'});click();if(!ev('intro.r')||ev('unis[caps[1]].tap')!==1)throw Error('click dash');ev('updateWorld(.08)');click();if(!ev('intro.dash')||ev('unis[caps[1]].tap')!==2)throw Error('chase chain');ev('intro.power=1;updateIntro(.1);intro.distract=1;updateIntro(.1);intro.rescue=1;updateIntro(.1);LM[0].os.forEach(o=>o.hp=0);update(.016)');if(ev('state')!=='between')throw Error('tutorial objective gate');
}
{
 const {ev,fire,click}=game();ev('startLevel(1)');if(ev('LM.length')!==5)throw Error('landmark count');let hp=ev("LM[4].os[0].hp");ev("hitObj(unis[0],LM[4].os[0],999)");if(ev("LM[4].os[0].hp")!==hp)throw Error('town hall shield');ev("LM.slice(0,-1).forEach(l=>l.os.forEach(o=>o.hp=0));hitObj(unis[0],LM[4].os[0],10)");if(ev("LM[4].os[0].hp")>=hp)throw Error('town hall unlock');ev('runTime=19;update(.016)');if(ev('wave')!==2||ev('cleaners.length')!==2)throw Error('act2 cleanup');fire('keydown',{code:'KeyD'});ev('updateWorld(.1)');fire('keyup',{code:'KeyD'});if(ev('caps[0]')!==1||ev('unis[0].order')<3)throw Error('left smart next');click();ev('updateWorld(.08)');click();if(ev('unis[caps[1]].tap')!==2||ev('unis[caps[1]].boost')<1)throw Error('2x prism');ev('updateWorld(.08)');click();if(ev('caps[1]')!==4||ev('unis[3].order')<3)throw Error('3x smart next');ev('update(.016)');if(ev('wave')!==3||ev('cleaners.length')!==4)throw Error('act3');ev('caps[0]=0;unis[1].order=3;unis[2].distract=1;cycle(0)');if(ev('caps[0]')!==2||ev('cueWhy[0]')!==1)throw Error('director reason');
}
{
...
console.log('headless top-10 smoke: PASS');
