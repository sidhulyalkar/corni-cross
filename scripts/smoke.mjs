...
let ws=fs.readFileSync('src/whip.js','utf8'),es=fs.readFileSync('src/expansion.js','utf8'),rs=fs.readFileSync('src/render.js','utf8'),hs=fs.readFileSync('src/herd.js','utf8');if(!ws.includes('X.arc(-7,7,14+i*7,.2,5.6)')||!ws.includes('quadraticCurveTo')||ws.includes('*700-56')||!ws.includes('whip.l*520'))throw Error('snap whip regression');if(!ws.includes("if(n<2){msg='WHIP 2X → DASH'")||ws.includes("msg='FOLLOW LESSON'"))throw Error('two-whip dash contract regression');if(!rs.includes('u.h+120')||!hs.includes('f.r+155')||!es.includes("lmText+=' • +6s'")||!es.includes('FINAL CLEANUP')||!es.includes('paintPct()>=stageGoal&&(dmask&15)===15'))throw Error('mane/distraction/takeover regression');
