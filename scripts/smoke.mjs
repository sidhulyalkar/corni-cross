...
{
 const {ev,fire}=game();ev("S.s('ccIntro10',1);state='title'");fire('keydown',{code:'Enter'});if(ev('level')!==0||ev('state')!=='play')throw Error('fresh solo tutorial not forced');if(ev('caps[0]')!==0||ev('caps[1]')!==-1)throw Error('tutorial must have one captain');
}
{
 const {ev,fire,press}=game();ev('startLevel(0)');if(ev('LM.length')!==1||ev("LM[0].k")!=='bakery')throw Error('tutorial landmark');let old=ev('caps[0]');fire('keydown',{code:'KeyD'});ev('updateWorld(.1)');ev('updateIntro()');if(ev('intro.step')!==1)throw Error('WASD lesson');fire('keyup',{code:'KeyD'});if(ev('caps[0]')!==old||ev('unis[0].order')>0)throw Error('WASD release must not auto-switch');press();ev('updateIntro()');if(ev('unis[0].power')!==1||ev('intro.step')!==2)throw Error('whip should charge active unicorn');press();press();if(ev('unis[0].power')!==3||!ev('intro.rescue'))throw Error('three whips should fill dash');fire('keydown',{code:'Space'});if(ev('unis[0].power')!==0||!ev('intro.dash')||ev('unis[0].dash')<=0)throw Error('space should spend dash charge');fire('keydown',{code:'ShiftLeft'});ev('updateIntro()');if(ev('caps[0]')===old||!ev('intro.power')||ev('intro.step')!==3)throw Error('manual shift lesson');if(ev(`unis[${old}].order`)<=0)throw Error('switch should leave outgoing route');ev('LM[0].os.forEach(o=>o.hp=0);update(.016)');if(ev('state')!=='between'||ev("S.g('ccIntro16')")!==1)throw Error('solo tutorial completion');
}
{
 const {ev,fire}=game();ev('startLevel(1);waveUp(2)');if(!ev("msg.includes('SHIFT')"))throw Error('manual switching not announced');if(ev('unis.filter(u=>u.live&&capSide(u.id)).length')!==1)throw Error('more than one captain');ev('paintStamp(unis[0],unis[0].x,unis[0].y,45);unis[0].vx=140;unis[0].vy=0;unis[0].a=0;unis[4].cool=12;suggested=bestIdle()');if(ev('suggested')!==4||!ev('roadAt(unis[0])'))throw Error('AI suggestion/highway state');fire('keydown',{code:'ShiftLeft'});if(ev('caps[0]')!==4||ev('unis[0].order')<5||!(ev('dmask')&16))throw Error('AI-assisted manual switch or highway route extension');ev('unis[1].cool=15;suggested=bestIdle()');fire('keydown',{code:'ShiftRight'});if((ev('dmask')&48)!==48)throw Error('second manual switch not registered');ev('update(.016)');if(ev('wave')!==3||ev('cleaners.length')!==4)throw Error('manual switches should unlock full herd');
}
{
 const {ev,click,fire}=game();ev('startLevel(1)');click();click();click();if(ev('unis[caps[0]].power')!==3||ev('chains')!==1)throw Error('3X whip should charge prism dash and preserve mastery chain');let c=ev('caps[0]');fire('keydown',{code:'Space'});if(ev('caps[0]')!==c||ev('unis[caps[0]].power')!==0)throw Error('dash must not auto-switch');
}
{
 const {ev,fire}=game();ev('startLevel(1);LM.slice(0,-1).forEach(l=>l.os.forEach(o=>o.hp=0))');let hp=ev('LM[4].os[0].hp');ev('hitObj(unis[0],LM[4].os[0],999)');if(ev('LM[4].os[0].hp')!==hp||!ev("msg.includes('28% CHAOS')"))throw Error('town hall chaos gate');ev('painted=PW*PH*.4;hitObj(unis[0],LM[4].os[0],10)');if(ev('LM[4].os[0].hp')>=hp)throw Error('town hall unlock');ev('runTime=19;update(.016)');if(ev('wave')!==2||ev('cleaners.length')!==2)throw Error('act2 cleanup');ev('stamp=6;cleaners[0].spray=0;updateCleaners(.01)');if(ev('cleaners[0].spray')>.17)throw Error('adaptive cleanup');fire('keydown',{code:'ShiftLeft'});ev('hud()');
}
{
...
console.log('headless v0.23 solo captain + rainbow highways smoke: PASS');
