...
const _titleW=title;title=function(){_titleW();if(S.g('ccIntro16')){X.fillStyle='#d8e9ff';text(`A/D WORLD • ${ZN[zone]} • ${plus?'HEAT '+(heat+1):'★'.repeat(S.g('ccC'+zone))}`,W/2,470,13,'center')}};
const _endW=end;end=function(){_endW();if(!level)return;let c=grade(),s=plus?'HEAT '+(heat+(landWin?0:1)):'★'.repeat(c)+'☆'.repeat(3-c);X.fillStyle='rgba(5,8,15,.88)';rr(W/2-245,382,490,44,13);X.fillStyle='#ffe56d';text(`${ZN[zone]} • ${s}`,W/2,410,16,'center')};
