...
function drawWorld(){
  ...
  for(let p of props){let x=p.x-cam;if(x<-300||x>W+300)continue;if(p.t==='cross'){X.fillStyle='#e8e2d7';for(let y=177;y<545;y+=42)X.fillRect(x-p.w/2,y,p.w,16);X.fillStyle='#111622';rr(x+p.w/2+12,112,25,38,6);X.fillStyle=crossGo(p.i)?'#72ef9b':'#ff6868';X.beginPath();X.arc(x+p.w/2+24,125,7,0,TAU);X.fill();X.fillStyle='#d8deed';text(crossGo(p.i)?'GO':'WAIT',x+p.w/2+24,146,7,'center')}else if(p.t==='fountain'){X.fillStyle='#7a7d83';X.beginPath();X.arc(x,p.y,p.r+10,0,TAU);X.fill();X.fillStyle='#214b63';X.beginPath();X.arc(x,p.y,p.r,0,TAU);X.fill();X.strokeStyle='#82d8ff';X.lineWidth=3;X.beginPath();X.arc(x,p.y,p.r*.45,0,TAU);X.stroke()}else if(p.t==='cart'){X.fillStyle='#f4c95d';rr(x-30,p.y-22,60,44,8);X.fillStyle='#dd5b4d';X.fillRect(x-26,p.y-32,52,10);X.fillStyle='#0d111b';X.beginPath();X.arc(x-20,p.y+26,7,0,TAU);X.arc(x+20,p.y+26,7,0,TAU);X.fill()}else if(p.t==='cones'){X.fillStyle='#f07842';for(let q=-p.w/2;q<p.w/2;q+=42){X.beginPath();X.moveTo(x+q,p.y-20);X.lineTo(x+q-10,p.y+18);X.lineTo(x+q+10,p.y+18);X.fill()}}}
  // destination parade: the final stretch turns the whole town into a celebration
  let fxp=finish-cam;if(fxp<W+560){for(let i=0;i<24;i++){let px=fxp-520+(i%12)*47,py=i<12?135:585;X.fillStyle=`hsl(${i*47} 70% 65%)`;X.beginPath();X.arc(px,py,6,0,TAU);X.fill();X.fillStyle='#25293a';X.fillRect(px-5,py+6,10,14)}if(player.x>finish-650)for(let i=0;i<28;i++){let px=fxp-580+(i*83+clock*55)%690,py=(i*57+clock*85)%H;X.fillStyle=`hsl(${i*61+clock*30} 90% 65%)`;X.fillRect(px,py,4,7)}}if(fxp<W+150){X.strokeStyle='#ffd35c';X.lineWidth=18;X.beginPath();X.arc(fxp,360,120,Math.PI,TAU);X.stroke();X.fillStyle='#fff';text('RAINBOW PARADE',fxp,315,20,'center');}
...
function drawPlayer(){let x=player.x-cam,y=player.y;X.save();X.translate(x,y);X.rotate(player.a);if(mouse.r){X.fillStyle='rgba(255,105,205,.12)';X.beginPath();X.moveTo(0,0);X.arc(0,0,125,-.72,.72);X.closePath();X.fill()}X.strokeStyle=mouse.r?'#ff89d7':mouse.l?'#7ee9ff':'#cad1e8';X.globalAlpha=.55;X.lineWidth=2;X.beginPath();X.arc(0,0,mouse.r?125:mouse.l?150:26,-.75,.75);X.stroke();X.globalAlpha=1;X.fillStyle='#151929';X.beginPath();X.arc(0,0,15,0,TAU);X.fill();X.fillStyle='#fff';X.beginPath();X.arc(6,-5,5,0,TAU);X.fill();X.fillStyle='#ff63b7';X.beginPath();X.moveTo(9,-10);X.lineTo(21,-18);X.lineTo(13,-5);X.fill();X.restore();if(mouse.l){X.strokeStyle='rgba(126,233,255,.38)';X.setLineDash([8,8]);X.beginPath();X.moveTo(x,y);X.lineTo(mouse.x,mouse.y);X.stroke();X.setLineDash([]);X.beginPath();X.arc(mouse.x,mouse.y,15+Math.sin(clock*9)*4,0,TAU);X.stroke()}if(mouse.l||mouse.r){X.fillStyle=mouse.l?'#7ee9ff':'#ff89d7';text(mouse.l?'WHISTLE':'SHOO',x,y-33,10,'center')}}
